...
void TcpConnection::send(const Train & msg)
{
	_socketIo.writen(msg.buff, msg.len);
}


void TcpConnection::sendInLoop(const Train & msg)
{ 
	_loop->runInLoop(std::bind(&TcpConnection::send, this, msg));//绑定发送函数以及对象本身发给对应线程
}
