...
//从缓冲区中获取数据
vector<string> LRUCache::get(string key){
    auto it = _keys.find(key);
    if(it == _keys.end()){
        cout<<"not found"<<endl;
        ...
        cout<<"found!"<<endl;
        for(auto it1 = it->second->value.begin(); it1 != it->second->value.end(); ++it1)//段错误
                        cout<<(*it1)<<" ";
        cout<<endl;
        vector<string> ret = it->second->value;
...
void LRUCache::update(const LRUCache& rhs){
    for(auto& i : rhs._keys){
           
        _keys.insert(i);//假设在一个缓存中有个元素处于链表尾的位置意味着
