...
void EventLoop::waitEpollFd()
{
	...
	do {
        ...
        //efd为引用的epoll实例上的事件，events指针指向的内存包含对调用者有效的事件
        //当达到maxevents时返回epoll_wait()会返回，maxevents必须大于零
        //调用等待最长时间是timeout
	...
	if(nready == -1) {//异常情形
		perror("epoll_wait");
		return;
	} else if(nready == 0) {//当达到timeout = 5000 时，视为超时时间
		...
		for(int idx = 0; idx != nready; ++idx) {//遍历epoll中所有的文件描述符，找到与接收的描述符相同的
